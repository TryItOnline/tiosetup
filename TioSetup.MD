...
We will use Fedora 24 for the arena server.
...
```Shell
yum install git psmisc selinux-policy-sandbox policycoreutils-sandbox vim-common setroubleshoot
...
```Shell
adduser runner
setfacl -m user:runner:rwx /srv
setfacl -m user:runner:rwx /opt
sudo -u runner -i
mkdir -p ~/.ssh
