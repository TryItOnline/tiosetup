...
**There were recent changes in the TIO setup process recently. This change has not been reflected in the README.md below yet. Watch this space.**
...
`$TMPDIR` mounted on tmpfs|`$TMPDIR` not mounted on tmpfs|Vultr mounts `/tmp` on tmpfs. This is problematic for some setup scripts because they quickly run out of the temp space. For this reason `tmp.mount` is masked in the script.
